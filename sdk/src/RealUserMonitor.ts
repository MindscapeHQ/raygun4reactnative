...
export default class RealUserMonitor {
  ...
  handleRequestOpen(method: string, url: string, xhr: any) {
    ...
    // Store the ID on the request, so the send and response callbacks can find it
    xhr._id_ = id;
    // Store the ID and the action taken on the device in a map, ID => REQUEST_META. The name upper-cases the method,
    // so `post` and `POST` share a name, and leaves out the query string and fragment, as Raygun's other SDKs do: they
    // can carry tokens, and split one endpoint into many names
    this.requests.set(id, { name: `${method.toUpperCase()} ${url.split(/[?#]/)[0]}` });
  }
  ...
  handleRequestSend(xhr: any) {
    // Extract the request's ID. Use that to get the RequestMeta object from the map
    const { _id_ } = xhr;
